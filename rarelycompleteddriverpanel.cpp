...
inline travelinfo* DRV_TD_id(int nalltravel)
{
	...
	do
	{
		l = input(0, 6, 42, screen.Y - 3, 48, screen.Y - 3, DRV_backcolor, DRV_errorcolor, "bpzEn");
		if (l == "~E") exit(0);
		else if (l == "~z" || l == "~b")
		{
		...
		else if (l == "~p") pnl_drivers(screen.X - 11, screen.Y - 10);
		else if (l == "") l = "0";
...
inline string find_chairs(travelinfo *t)
{
	string l;
	long int id_travel;
	...
	while (getline(chairs, l))
	{
		...
		s << l;
		s >> id_travel;
		if (id_travel == t->id) return l;
	}
}
inline void passengerinfo(string chairs)
{
	stringstream s;
	long int id;
	int capacity;
...
inline void DRV_ET_day(int x, int y, int nalltravel)
{
	...
	do
	{
		do
		{
			...
			if (day == "~p") pnl_drivers(screen.X - 11, screen.Y - 10);
...
inline void DRV_ET_month(int x, int y, int nalltravel)
{
	...
	do
	{
		do
		{
			...
			if (month == "~p") pnl_drivers(screen.X - 11, screen.Y - 10);
...
inline void DRV_ET_year(int x, int y, int nalltravel, travelinfo *t)
{
	...
	do
	{
		do
		{
			...
			if (year == "~b" || year == "~p") pnl_drivers(screen.X - 11, screen.Y - 10);
...
inline void DRV_ET_hour(int x, int y, int nalltravel, travelinfo *t)
{
	...
	do
	{
		do
		{
			...
			if (hour == "~p") pnl_drivers(screen.X - 11, screen.Y - 10);
...
inline void DRV_ET_minute(int x, int y, int nalltravel, travelinfo *t)
{
	...
	do
	{
		do
		{
			...
			if (minute == "~p") pnl_drivers(screen.X - 11, screen.Y - 10);
...
void edit_travel(int length, int width, int nalltravel)
{
	...
	t->delay += (DRV_edit.year - t->year) * 8760 + (DRV_edit.month - t->month) * 720 + (DRV_edit.day - t->day) * 24 + (DRV_edit.hour - t->hour);
...
inline void DRV_NT_i(int x, int y)
{
	...
	do
	{
		do
		{
			...
			if (i == "~b" || i == "~p" || i == "~z") pnl_drivers(screen.X - 11, screen.Y - 10);
			else if (i == "~E") exit(0);
		} while (i[0] == '~');
...
inline void DRV_NT_f(int x, int y)
{
	...
	do
	{
		do
		{
			...
			if (f == "~b" || f == "~p") pnl_drivers(screen.X - 11, screen.Y - 10);
			else if (f == "~z")
			{
				print(' ', DRV_acceptcolor, x + 1, y + 3);
...
inline void DRV_NT_day(int x, int y)
{
	...
	do
	{
		...
		{
			day = input(2, 2, x + 8, y + 5, x + 4, y + 6, DRV_backcolor, DRV_errorcolor, "bpzEn");
			if (day == "~b" || day == "~p") pnl_drivers(screen.X - 11, screen.Y - 10);
			else if (day == "~z")
			{
				print(' ', DRV_acceptcolor, x + 1, y + 5);
				print(' ', DRV_backcolor, x + 4, y + 6, 30);
...
inline void DRV_NT_month(int x, int y)
{
	...
	do
	{
		...
		{
			month = input(2, 2, x + 11, y + 5, x + 4, y + 6, DRV_backcolor, DRV_errorcolor, "bpzEn");
			if (month == "~b" || month == "~p") pnl_drivers(screen.X - 11, screen.Y - 10);
			else if (month == "~z")
			{
				print(' ', DRV_acceptcolor, x + 1, y + 5);
				print(' ', DRV_backcolor, x + 4, y + 6, 30);
...
inline void DRV_NT_hour(int x, int y)
{
	...
	do
	{
		...
		{
			hour = input(2, 2, x + 8, y + 7, x + 4, y + 8, DRV_backcolor, DRV_errorcolor, "bpzEn");
			if (hour == "~b" || month == "~p") pnl_drivers(screen.X - 11, screen.Y - 10);
			else if (hour == "~z")
			{
				print(' ', DRV_acceptcolor, x + 1, y + 7);
				print(' ', DRV_backcolor, x + 4, y + 8, 30);
...
inline void DRV_NT_minute(int x, int y)
{
	...
	do
	{
		do
		{
			...
			if (minute == "~b" || minute == "~p") pnl_drivers(screen.X - 11, screen.Y - 10);
			else if (minute == "~z")
			{
				print(' ', DRV_acceptcolor, x + 1, y + 7);
				print(' ', DRV_backcolor, x + 4, y + 8, 30);
...
inline void DRV_ED_username(int x, int y)
{
	...
	while (1)
	{
		...
		do
		{
			...
			if (new_username == "~b" || new_username == "~p" || new_username == "~z") pnl_drivers(screen.X - 11, screen.Y - 10);
...
inline void DRV_ED_password(int x, int y)
{
	...
	do
	{
		...
		if (new_password == "~b" || new_password == "~p") pnl_drivers(screen.X - 11, screen.Y - 10);
...
inline void DRV_ED_confirmpassword(int x, int y)
{
	...
	while (1)
	{
		do
		{
			...
			if (pass == "~b" || pass == "~p") pnl_drivers(screen.X - 11, screen.Y - 10);
			else if (pass == "~z")
			{
...
inline void DRV_ED_name(int x, int y)
{
	...
	do
	{
		...
		if (new_name == "~b" || new_name == "~p") pnl_drivers(screen.X - 11, screen.Y - 10);
		else if (new_name == "~z")
		{
...
inline void DRV_ED_family(int x, int y)
{
	...
	do
	{
		...
		if (new_family == "~b" || new_family == "~p") pnl_drivers(screen.X - 11, screen.Y - 10);
...
inline void DRV_ED_phone(int x, int y)
{
	...
	do
	{
		...
		if (new_phone == "~b" || new_phone == "~p") pnl_drivers(screen.X - 11, screen.Y - 10);
		else if (new_phone == "~z")
		{
...
inline travelinfo* travelshow(bool showfiald, int n, travelinfo *first = &t0)
{
	...
	travelinfo *currenttravel = first;
	int i;
	for (i = first->num; currenttravel->next != nullptr && i <= n + first->num; currenttravel = currenttravel->next)
	{
		tm*timer = time();
...
{
	//==================================== Show ====================================
	int nalltravel = readTravel(user.id);
...
void login(int length, int width)
{
	...
	if (check)
	{
		...
		Sleep(1000);
		check == 1 ? pnl_user(screen.X - 16, screen.Y - 16, 115, 78) : pnl_drivers(screen.X - 11, screen.Y - 10);
	}
	else
	{
		background(79, screen);
